...
const useWeekIntentionsStore = create<WeekIntentionsStore>((set, get) => ({
  ...copyObject,
  updateActiveDay: (id) => {
    set({ activeDay: id });
  },
  ...
  updateWeek: (date: Date) => {
    ...
    getEnumDays.forEach((day, i) => {
      ...
      const foundDay = get().dayIntentions.get(i.toString());
      if (!foundDay) return;
      foundDay.dateOfDay = new Date(weekStart.setDate(weekStart.getDate() + i));
    });
...
export default useWeekIntentionsStore;
